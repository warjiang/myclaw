...
async def task_complete_tool(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "content": [
            {"type": "text", "text": f"Task {status}: {summary}"}
        ],
    ...
    async def query(self, prompt: str) -> AsyncGenerator[str, None]:
        ...
        async for msg in self.client.receive_response():
            ...
            if isinstance(msg, AssistantMessage):
                logger.debug(
                    "Processing AssistantMessage with {} content blocks",
                    len(msg.content)
                )
                for idx, block in enumerate(msg.content):
                    ...
                    if isinstance(block, TextBlock):
                        ...
                        if block.name == "mcp__utilities__task_complete":
                            task_completed = True
                            logger.info(
                                "Task complete tool called with: {}",
                                block.input
                            )
                ...
                logger.debug(
                    "Received UserMessage with {} content blocks",
                    len(msg.content)
                )
